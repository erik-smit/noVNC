...
function Display(defaults) {
    ...
    that.get_context = function() {
        return c_ctx;
    };

    that.set_scale = function(scale) {
        rescale(scale);
    };

    that.set_width = function(val) {
        that.resize(val, fb_height);
    };
    that.get_width = function() {
        return fb_width;
    };

    that.set_height = function(val) {
        that.resize(fb_width, val);
    };
    ...
    function constructor() {
        ...
        if (UE.gecko) {
            Util.Debug("Browser: gecko " + UE.gecko);
        }
        if (UE.webkit) {
            Util.Debug("Browser: webkit " + UE.webkit);
        }
        if (UE.trident) {
            Util.Debug("Browser: trident " + UE.trident);
        }
        if (UE.presto) {
            Util.Debug("Browser: presto " + UE.presto);
        }
    ...
    that.viewportChange = function(deltaX, deltaY, width, height) {
        ...
        if ((v.w !== width) || (v.h !== height)) {
            ...
            if (v.w > 0 && v.h > 0 && c.width > 0 && c.height > 0) {
                saveImg = c_ctx.getImageData(0, 0, (c.width < v.w) ? c.width : v.w, (c.height < v.h) ? c.height : v.h);
            }
...
function changeCursor(target, pixels, mask, hotx, hoty, w0, h0, cmap) {
    "use strict";
    var cur = [],
        rgb, IHDRsz, RGBsz, ANDsz, XORsz, url, idx, alpha, x, y;
