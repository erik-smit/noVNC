        ...
        switch (rfb_state) {
            ...
            case 'Security':
                if (rfb_version >= 3.7) {
                    ...
                    if (ws.rQwait("security type", num_types, 1)) {
                        return false;
                    }
                    if (num_types === 0) {
                        strlen = ws.rQshift32();
                    ...
                    // Server decides
                    if (ws.rQwait("security scheme", 4)) {
                        return false;
                    }
            ...
            case 'Authentication':
                ...
                switch (rfb_auth_scheme) {
                    ...
                    case 2: // VNC authentication
                        ...
                        if (ws.rQwait("auth challenge", 16)) {
                            return false;
                        }
                    ...
                    case 16: // TightVNC Security Type
                        if (conf.rfb_aten) {
                            ...
                            Util.Info(ws.rQshiftStr(24)); // padding
                            updateState('SecurityResult');
                        } else {

                            if (ws.rQwait("num tunnels", 4)) {
                                return false;
                            }
                            ...
                            for (var i = 0; i < subAuthCount; i++) {
                                ...
                                if (ws.rQwait("sub auth capabilities " + i, 16)) {
                                    return false;
                                }
            ...
            case 'SecurityResult':
                if (ws.rQwait("VNC auth response ", 4)) {
                    return false;
                }
                switch (ws.rQshift32()) {
                    case 0: // OK
            ...
            case 'ServerInitialisation':
                if (ws.rQwait("server initialization", 24)) {
                    return false;
                }
                /* Screen size */
    ...
    : function() {
        ...
        switch (msg_type) {
            ...
            case 3: // ServerCutText
                ...
                if (ws.rQwait("ServerCutText header", 7, 1)) {
                    return false;
                }
                ws.rQshiftBytes(3); // Padding
                length = ws.rQshift32();
                if (ws.rQwait("ServerCutText", length, 8)) {
                    return false;
                }
...
//            Util.Debug(">> FBU: ws.rQlen: " + ws.rQlen());
        ...
        while (FBU.rects > 0) {
            ...
            if (ws.rQwait("FBU", FBU.bytes)) {
                return false;
            }
            if (FBU.bytes === 0) {
                if (ws.rQwait("rect header", 20)) {
                    return false;
                }
                /* New FramebufferUpdate */

                hdr = ws.rQshiftBytes(20);
                FBU.x = (hdr[0] << 8) + hdr[1];
                FBU.y = (hdr[2] << 8) + hdr[3];
...
*/
    ...
    encHandlers.HEXTILE = function display_hextile() {
        ...
        while (FBU.tiles > 0) {
            FBU.bytes = 1;
            if (ws.rQwait("HEXTILE subencoding", FBU.bytes)) {
                return false;
            }
            ...
            if (subencoding & 0x01) { // Raw
                ...
                if (subencoding & 0x08) { // AnySubrects
                    ...
                    if (ws.rQwait("hextile subrects header", FBU.bytes)) {
                        return false;
                    }
            ...
            if (ws.rQwait("hextile", FBU.bytes)) {
                return false;
            }
    ...
    function display_tight(isTightPNG) {
        ...
        if (ws.rQwait("TIGHT compression-control", FBU.bytes)) {
            return false;
        }
        ...
        var handlePalette = function() {
            ...
            if (ws.rQwait("TIGHT palette " + cmode, FBU.bytes)) {
                return false;
            }
            ...
            if (ws.rQwait("TIGHT " + cmode, FBU.bytes)) {
                return false;
            }
        ...
        switch (cmode) {
            ...
            case "filter":
                FBU.bytes += 2;
                break; // filter id + num colors if palette
            case "copy":
                break;
        }
    ...
    encHandlers.Cursor = function set_cursor() {
        ...
        if (ws.rQwait("cursor encoding", FBU.bytes)) {
            return false;
        }
...
//        arr.push8(0); // padding
    ...
    that.sendCtrlAltDel = function() {
        if (rfb_state !== "normal" || conf.view_only) {
            return false;
        }
